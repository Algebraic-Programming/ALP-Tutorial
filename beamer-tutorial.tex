...
\begin{itemize}
\item Supersteps $i = 0..n$ with participants $s = 0..p(l)$
\item Work vectors at the leaf level: $w_v(i,s,l=0) = (w_{\text{scalar}}, w_{\text{vec\_SIMD}})$
\item Stream descriptors $ks(i,s,l)$ for disjoint memory regions (0..$kmax(l)$)
\item Traffic descriptors $t(i,s,l,k)$, $v(i,s,l,k)$ for bytes sent/received
\end{itemize}
\vspace{0.5em}
\begin{columns}[t]
\column{0.48\textwidth}
\textbf{Per-superstep computation cost}\\[0.2em]
\[
    c_{\text{comp}} = \max\left( \frac{r_{\text{scalar}}}{w_{\text{scalar}}}, \frac{w_{\text{vec\_SIMD}}}{ops_{\text{SIMD}}} \right)
\]
\column{0.48\textwidth}
\textbf{Per-level communication cost}\\[0.2em]
\[
