...
\begin{frame}{HIPO Modeling}
\framesubtitle{k-Multi-BSP: Extending Multi-BSP with 'k-Stream' logic}
\begin{itemize}
\item \textbf{Hierarchy}: retain Multi-BSP levels and supersteps
\begin{itemize}
    \item Levels map to cores $\rightarrow$ caches $\rightarrow$ numa hierarchy ( $\rightarrow$ nodes? - TBD)
    \item Keep the bulk-synchronous execution semantics (supersteps etc)
\end{itemize}
\item \textbf{$k$-streams concept}: Model concurrent accesses to different memory regions
\begin{itemize}
    \item Streams are the theoretical parallel access paths within a superstep
    \item Capture consecutive vs. non-consecutive access patterns
\end{itemize}
\item \textbf{Thread count/allocation} $\rightarrow$ different hardware hierarchies
\begin{itemize}
    \item Different thread counts or affinity treated as \textit{different hardware instances/systems}
    \item Enables exploring scheduling choices without expending the model's parameter space.
\end{itemize}
\item \textbf{For ALP primitives}: Must bridge implementations $\leftrightarrow$ hardware model
\begin{itemize}
    \item Currently kernels are not written in multi-BSP hierarchical/recursive logic.
    \item \textbf{Assumption:} Kernels matched to levels based on their working set (mem footprint).
    \begin{itemize}
      \item Use memory footprint to pick the lowest level $m_i$ that fits the working set.
      \item Optionally incorporate thread allocation to infer NUMA effects by swapping levels
    \end{itemize}
\end{itemize}
\end{itemize}
% Owner: PA
...
\begin{frame}{HIPO Modeling}
\framesubtitle{k-Multi-BSP Model notation (Work in Progress)}
\footnotesize
\begin{columns}[t]
\column{0.48\textwidth}
\textbf{Hardware parameters}
\begin{itemize}
    \item Levels $l = 0..d$ with $r_v(l) = (r_{\text{scalar}}, r_{\text{vec\_SIMD}})$
    \item Bandwidth $g(l)$ (inverse BW in s/access), latency $ls(l)$
    \item Memory budget $m(l)$, sub-components $p(l)$
...
\begin{itemize}
    \item Supersteps $i = 0..n$, participants $s = 0..p(l)$
    \item Work vectors $w_v(i,s,l=0) = (w_{\text{scalar}}, w_{\text{vec\_SIMD}})$
    \item Streams $ks(i,s,l)$ accessing disjoint regions
    \item Traffic descriptors $t(i,s,l,k)$, $v(i,s,l,k)$ in bytes sent/received
\end{itemize}
...
\end{equation*}
\textbf{Simplified memory-bound variant}: assume uniform per process, drop $r_v$, $w_v$, $s$; cost reduces to $T = \sum_{i,l,k} c_{\text{comm}}(i,l)$ with $6d+1$ HW params and $1 + n d (ks+1)$ algo params.
% Owner: PA
\end{frame}
